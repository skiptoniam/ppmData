...
snail1 <- subset(snails,SpeciesID=='Victaphanta lampra')
...
fm <- gam(pres/wts ~ s(X,k = 9) + s(Y,k = 9) + s(max_temp_hottest_month,k = 9) +
            s(annual_mean_precip,k = 9) + s(annual_mean_temp,k = 9),
              weights = dat$wts,
              data = dat,
              family = poisson())
plot(fm)
...
# 
p_cell <- p*(area(preds)*1e6)#prod(bkgrid$parameters$newresolution)
