...
If we assume that there is no sampling bias, then by estimating $\lambda(z)$ we are also estimating $\tilde \lambda(z)$, despite this assumption, we can only every obtain relative probabilities of occurrence (see Phillips and Elith 2013). Effectively this is what we have achieved in the sections above. 

An alternative approach is to assume the $s$ is an unknown function of $z$, but are known to depend on $\tilde \lambda(z)$ in this case, we could model $z$ and $s$ as log-linear features of the covariates $x_1(z)$ and $x_2(z)$. Where $x_1(z)$ are covariates that contribute to our understanding of $\beta$, how our species response to covariates (the slopes).   
...
Now that we have estimated $s(z)$ we can use it as an offset in the estimation of $ \lambda(z)$. If we refer to Fithian and Hastie 2014, we can see back in section 2.5 *Identifiability and sampling bias* that $\lambda(z) = \tilde \lambda(z) \cdot s(z)$, so we can directly translate this into an offset within a model that aims to estimated the *observation process* for *Victaphanta lampra*. This is because:

$$\lambda(x)= e^{\tilde \alpha + \tilde\beta^\prime x_1(z)} e^{\gamma^\prime + \delta x_2(x)}$$
