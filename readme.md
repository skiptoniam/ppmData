...
*p**r*(*y* = 1)=1 − *p**r*(*y* = 0) =1 − *e**x**p*(−*s*(*z*)\**A*)
...
Now that we have estimated *s*(*z*) we can use it as an offset in the estimation of *λ*(*z*). If we refer to Fithian and Hastie 2014, we can see back in section 2.5 *Identifiability and sampling bias* that $\\lambda(z) = \\tilde \\lambda(z) \\cdot s(z)$, so we can directly translate this into an offset within a model that aims to estimated the *observation process* for *Victaphanta lampra*. This is because:

$\\lambda(x)=\\exp^{\\tilde \\alpha + \\tilde\\beta^\\prime x\_1(z)} \\exp^{\\gamma^\\prime + \\delta x\_2(x)}$
