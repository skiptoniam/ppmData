...
```{r fig1, fig.width=6, fig.height=3,fig.align='center',cache=TRUE, warning=FALSE, echo=FALSE, message=FALSE,fig.cap = "Quadrature schemes generated using the ppmData package for the species \\textit{Tasmaphena sinclairi} using Tasmania, Australia, as our spatial region. Red points are the known presences of \\textit{Tasmaphena sinclairi}. Black points are the background quadrature locations. A) Quasi-random quadrature where the integration points are generated using a quasi-random areal sample. Weights for each integration point are calculated as the dual of the subsequent Dirichlet tessellation of both the presence and integration points. B) Pseudo-random quadrature generated using random points from within the window. C) Grid quadrature where quadrature points are generated on a regular grid."}
library(ppmData)
...
ppmdata1 <- ppmData(npoints = npoints, presences=presences,
                    ...
                    quad.method='quasi.random')
ppmdata2 <- ppmData(npoints = npoints, presences=presences,
                    window = preds[[1]], covariates=preds,
                    quad.method='pseudo.random')
ppmdata3 <- ppmData(npoints = npoints, presences=presences,
                    window = preds[[1]], covariates=preds,
                    quad.method='grid')

# par(mar=c(7,7,7,7))
tassie <- ozmaps::ozmap_states[ozmaps::ozmap_states$NAME=="Tasmania",]

layout(mat=matrix(1:3, nrow=1, ncol=3), heights=c(1,1), widths=c(1,1,1))
plot(ppmdata1, main="",plt.legend=FALSE,pch.cols='tomato',rcols="blanchedalmond",cex=1)
plot(sf::st_geometry(tassie),add=TRUE, border=gray(0.2,0.5))
mtext("A)", side=2, line=1, at=par('usr')[4], las=2)
plot(ppmdata2, main="",plt.legend=FALSE,pch.cols='tomato',rcols="blanchedalmond",cex=1)
plot(sf::st_geometry(tassie),add=TRUE, border=gray(0.2,0.5))
mtext("B)", side=2, line=1, at=par('usr')[4], las=2)
plot(ppmdata3, main="",plt.legend=FALSE,pch.cols='tomato',rcols="blanchedalmond",cex=1)
plot(sf::st_geometry(tassie),add=TRUE, border=gray(0.2,0.5))
mtext("C)", side=2, line=1, at=par('usr')[4], las=2)
```
...
```{r multiple species quad, echo=FALSE, cache=TRUE, warning=FALSE, message=FALSE, fig.cap='Multiple species quadrature scheme. Here we can see the presences of five different snail species across Tasmania. For the multiple species quadrature there is a common set of background quadrature sites, but the object returns species specific weights.',fig.align='center',fig.height=5,fig.width=5}
library(ppmData)
path <- system.file("extdata", package = "ppmData")
lst <- list.files(path=path,pattern='*.tif',full.names = TRUE)
preds <- rast(lst)
presences <- snails
preds <- scale(preds)
ppmdata.multi <- ppmData(npoints = 1000, presences = presences,
                   window = preds[[1]], covariates = preds)
tassie <- ozmaps::ozmap_states[ozmaps::ozmap_states$NAME=="Tasmania",]
plot(ppmdata.multi, main = '',rcols="blanchedalmond",
     pch.cols = RColorBrewer::brewer.pal(5,"Set1"), pch=19, cex=0.8)
plot(sf::st_geometry(tassie),add=TRUE, border=gray(0.2,0.5))
```
...
```{r fit model, cache=TRUE, warning=FALSE, message=FALSE}
...
```{r, fig.cap='Predicted intensity per for $\\approx $ 0.6 km$^2$ sized raster cell for the snail species \\textit{Tasmaphena sinclairi}. The points are the recoreded presences of \\textit{Tasmaphena sinclairi} in the region.', cache = TRUE, fig.align='center',fig.height=5,fig.width=5, echo=FALSE}
library(terra)
path <- system.file("extdata", package = "ppmData")
lst <- list.files(path=path, pattern='*.tif',full.names = TRUE)
preds <- rast(lst)
xy <- ppmFit::xyFromWindow(preds[[1]], mask.na = TRUE)
preds <- scale(preds)
preds2 <- c(xy,preds) 
pred <- predict(preds2, ft.ppm, type='response')
main <- ""#expression(paste("Predicted intensity per cell: ",lambda[i]))
p <- pred*prod(res(pred))
qtiles <- quantile(p[!is.na(p)],probs=seq(0,1,len=10),na.rm=TRUE)
breaks <- cut(p[!is.na(p)], breaks = qtiles,dig.lab = 2)

plot(p, breaks = qtiles,col=rev(hcl.colors(11,palette= "Spectral")),
     main=main,plg=list(cex=0.5,legend=levels(breaks),
                        title=expression(paste(lambda[i])),x=142,y=-43),axes=FALSE)
tassie <- ozmaps::ozmap_states[ozmaps::ozmap_states$NAME=="Tasmania",]
plot(sf::st_geometry(tassie),add=TRUE, border=gray(0.2,0.5))
points(presences[!duplicated(presences),],col=adjustcolor("black",alpha.f = 0.5),pch=16,cex=0.8)
```
